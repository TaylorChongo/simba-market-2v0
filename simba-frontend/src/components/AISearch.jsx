  ...
  return (
    <div className="relative w-full md:max-w-xl mx-0 md:mx-4" ref={containerRef}>
      ...
      <form 
        ...
        className={`relative flex items-center transition-all duration-300 ${isExpanded ? 'scale-[1.02] opacity-0 md:opacity-100' : ''}`}
      >
      ...
      {isExpanded && (
        <div className="fixed inset-0 z-[100] bg-black/40 md:bg-transparent backdrop-blur-sm md:backdrop-blur-none md:absolute md:top-full md:inset-auto md:left-0 md:right-0 md:mt-3 flex items-end md:items-start justify-center animate-in fade-in duration-300">
          <div className="bg-surface w-full h-[90vh] md:h-[70vh] rounded-t-[32px] md:rounded-[24px] border-t md:border border-outline-variant shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom md:slide-in-from-top-2 duration-500 md:duration-300">
            
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 md:px-4 md:py-3 border-b border-outline-variant bg-surface-container-low shrink-0">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-surface-container-lowest p-6 md:p-4 space-y-6">
              {messages.length === 0 && !loading && (
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-on-surface-variant max-w-[200px] mx-auto mt-1">Try searching for recipes, categories, or specific items.</p>
                  </div>
                </div>
              )}
              ...
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  {/* Message Bubble */}
                  <div className={`max-w-[90%] md:max-w-[85%] rounded-2xl p-4 shadow-sm relative ${
                    msg.role === 'user' 
                    ...
                    <p className={`text-[13px] md:text-sm ${msg.role === 'user' ? 'font-medium' : 'font-semibold leading-relaxed'}`}>
                      {msg.text}
                    </p>
                  </div>
                        ...
                        {msg.products.map((product) => (
                          ...
                          >
                             <div className="w-16 h-16 rounded-xl overflow-hidden bg-white shrink-0 border border-outline-variant/50">
                                <img 
                                  src={optimizeCloudinaryUrl(product.image, { width: 150, height: 150 })} 
                                  alt={product.name}
            ...
            {/* Sticky bottom input in the modal */}
            <div className="p-6 md:p-4 bg-surface border-t border-outline-variant shrink-0 pb-10 md:pb-4">
               <form 
                 onSubmit={handleSearch}
               ...
               >
                  <input
                    ...
                    className="w-full bg-surface-container-low border border-outline-variant rounded-2xl pl-4 pr-14 py-3.5 md:py-3 text-[13px] md:text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all shadow-inner"
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                  />
                  <button 
                    type="submit"
                    disabled={loading || !query.trim()}
                    className="absolute right-2 p-2.5 bg-primary text-on-primary rounded-xl disabled:opacity-50 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-primary/20"
                  >
                    {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
                  </button>
               </form>
               <p className="text-[10px] text-center text-outline mt-3 font-bold uppercase tracking-widest hidden md:block">
                 AI may provide inaccurate product details. Verify at checkout.
               </p>
            </div>
          </div>
        </div>
