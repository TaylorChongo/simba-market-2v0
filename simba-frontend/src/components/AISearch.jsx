...
import { Send, Loader2, X, Sparkles, Plus, Minimize2 } from 'lucide-react';
...
const AISearch = ({ placeholder, autoOpen = false, onClose }) => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/ai-search`, {
        ...
        body: JSON.stringify({ query: userMessage.text, history: newMessages }),
      });
  ...
  const ChatPanel = ({ chatEndRefProp }) => (
    <div className="relative bg-surface w-full rounded-t-[28px] md:rounded-[24px] border-t md:border border-outline-variant shadow-2xl flex flex-col h-[85dvh] md:h-[70dvh] animate-in slide-in-from-bottom md:slide-in-from-top-2 duration-300">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-outline-variant bg-surface-container-low rounded-t-[28px] md:rounded-t-[24px] shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={handleClose} title="Close" className="p-2 hover:bg-error/10 rounded-xl text-outline hover:text-error transition-all">
            <X className="w-5 h-5" />
          </button>
        </div>
      </div>
        ...
        {lastAi && !loading && lastAi.products?.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 space-y-3 text-center w-full">
            ...
            <div className="flex gap-2 mt-2">
              ...
              <button onClick={handleClose} className="px-3 py-2 bg-surface border border-outline-variant rounded-xl text-sm">Close</button>
            </div>
          </div>
        )}
  ...
  return (
    <div className="relative w-full md:max-w-xl mx-0 md:mx-4" ref={containerRef}>
      ...
      {/* Desktop panel — inline, absolute positioned relative to containerRef */}
      {isExpanded && (
        <div className="hidden md:block absolute top-full left-0 right-0 mt-3 z-[200] animate-in fade-in duration-300">
          <ChatPanel chatEndRefProp={chatEndRef} />
        </div>
      )}

      {/* Mobile panel — portal to escape backdrop-filter stacking context */}
      {isExpanded && createPortal(
        <div className="md:hidden fixed inset-0 z-[200] flex flex-col justify-end animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-black/40" onClick={() => setIsExpanded(false)} />
          <div className="relative w-full">
            <ChatPanel chatEndRefProp={mobileChatEndRef} />
          </div>
        </div>,
        document.body
      )}
