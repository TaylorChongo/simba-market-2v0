...
const InlineBranchMap = () => {
  ...
  const [mapCenter, setMapCenter] = useState(() =>
    userLocation ? userLocation : [-1.9441, 30.0619]
  );
  ...
  const branchesWithDistance = useMemo(() => {
    ...
    return branches
      ...
      .sort((a, b) => {
        const aD = a.route?.distanceKm ?? a.directDistance;
        const bD = b.route?.distanceKm ?? b.directDistance;
        return aD - bD;
      });
  }, [branches, branchRoutes, userLocation]);

  useEffect(() => {
    if (!isMapVisible || !userLocation) return;
    ...
    const fetchRoutes = async () => {
      ...
      const results = await Promise.allSettled(
        branches.map(async (branch) => {
          ...
          const url = `${ROUTE_SERVICE_URL}/${uLng},${uLat};${branch.lng},${branch.lat}?overview=full&geometries=geojson`;
          const res  = await fetch(url, { signal: controller.signal });
          if (!res.ok) throw new Error(`Route failed for ${branch.name}`);
          const data = await res.json();
          const route = data.routes?.[0];
          if (!route?.geometry?.coordinates?.length) throw new Error(`No route for ${branch.name}`);
          return [
            branch.name,
            {
      ...
      const loaded = Object.keys(nextRoutes).length;
      if (loaded === 0) {
        setRouteStatus('error');
        setRouteError('Road routes are temporarily unavailable.');
      } else if (loaded < branches.length) {
                ...
                <div className="flex justify-between items-center gap-2">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className={`truncate font-black text-[11px] leading-tight ${
                        selectedBranch === branch.name ? 'text-primary' : 'text-on-surface'
                      }`}>
                ...
                >
                  <Tooltip sticky>
                    <div className="text-xs font-bold">
                      {shortName(branch.name)}: {branch.route.distanceKm.toFixed(1)} km
                    </div>
                  </Tooltip>
                </Polyline>
              );
              ...
              >
                <Popup closeButton={false}>
                  <div className="p-2 text-center">
                    <p className={`font-black text-[9px] uppercase tracking-[0.2em] mb-1 ${
                      userLocation && index === 0 ? 'text-error' : 'text-primary'
                    }`}>
                      {userLocation && index === 0 ? 'Closest Branch' : 'Simba Branch'}
                    </p>
                    <p className="font-black text-xs text-on-surface">{shortName(branch.name)}</p>
                    {branch.route && (
                      <div className="mt-1 flex items-center justify-center gap-1 text-[10px] font-bold text-outline">
                        <Navigation className="h-3 w-3" />
                        {branch.route.distanceKm.toFixed(1)} km
            ...
            <div className="max-w-md rounded-2xl border border-outline-variant bg-surface/90 px-4 py-3 shadow-2xl backdrop-blur-md">
              {userLocation ? (
                <div className="space-y-2">
                  ...
                  {routeError && (
                    <p className="text-[10px] font-bold text-warning">{routeError}</p>
                  )}
                  {branchesWithDistance[0]?.route ? (
                    <p className="text-xs font-black text-error">
                      Nearest: {shortName(branchesWithDistance[0].name)} ({branchesWithDistance[0].route.distanceKm.toFixed(1)} km)
                    </p>
                  ) : closestBranch && (
                    <p className="text-xs font-black text-error">
                      Nearest: {shortName(closestBranch.name)} ({closestBranch.distance?.toFixed(1)} km)
                    </p>
                  )}
                </div>
              ) : (
                <p className="text-xs font-bold text-outline">
                  {isFetchingLocation
                    ...
                    : locationError || 'Allow location access to show distances to each branch.'}
                </p>
              )}
            </div>
          </div>
